...
"""
        ...
        # ---------- QUERY INTENT: fetch and summarize plans ----------
        if intent == 'query':
            ...
            # Generate natural language summary in the user's language
            summary_prompt = f"""
...
"""
        ...
        if intent == 'find_slot':
            ...
            if free_slots:
                ...
                # No free slot – fall back to task
        ...
        # ---------- FALLBACK: Google Tasks (task intent) ----------
        tasks_service = build('tasks', 'v1', credentials=creds)
        task_lists = tasks_service.tasklists().list().execute()
